...
class GradientDescentSolver(Solver):
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        "*** YOUR CODE HERE ***"
        for i in range(len(param_vars)):
            updated_value = (self.momentum*vel_vars[i]) - (self.learning_rate*grad_tensors[i])
            updates.append((vel_vars[i], updated_value))
            updated_weight = param_vars[i] + updated_value
            updates.append((param_vars[i], updated_weight))
        #util.raiseNotDefined()
        return updates
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        val_generator = MinibatchIndefinitelyGenerator(val_data, 1, self.shuffle)
        ...
        for iter_ in range(self.iterations):
            ...
            inp, target = train_generator.next()
            train_loss = session.run(loss_tensor, feed_dict = {model.input_ph: inp, target_ph: target})
            session.run(update_ops, feed_dict = {model.input_ph: inp, target_ph: target})
            ...
            inp, target = val_generator.next()
            val_loss = session.run(loss_tensor, feed_dict = {model.input_ph: inp, target_ph: target})
            train_losses.append(train_loss)
            val_losses.append(val_loss)
            if callback is not None: callback(model)
            self.display_progress(iter_, train_losses, val_losses)
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        val_generator = MinibatchIndefinitelyGenerator(val_data, self.batch_size, self.shuffle)
